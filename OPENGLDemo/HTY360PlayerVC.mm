...
@property (strong, nonatomic) PanoramaVC *glkViewController;
...
- (IBAction)changBtnClicked:(id)sender {
   if(_perspectMode == ZLCPanoPerspectModeNormal)
   {
       _perspectMode = ZLCPanoPerspectModeFisheye;
       [_glkViewController setPerspectiveMode:_perspectMode];
   }else if(_perspectMode == ZLCPanoPerspectModeFisheye)
   {
       _perspectMode = ZLCPanoPerspectModeLittlePlanet;
       [_glkViewController setPerspectiveMode:_perspectMode];
   }else{
       _perspectMode = ZLCPanoPerspectModeNormal;
       [_glkViewController setPerspectiveMode:_perspectMode];
   }
...
- (CVPixelBufferRef)retrievePixelBufferToDraw {
    CVPixelBufferRef pixelBuffer = [self.videoOutput copyPixelBufferForItemTime:[self.playerItem currentTime] itemTimeForDisplay:nil];
      [_glkViewController refreshTexture:pixelBuffer];
...
- (void)configureGLKView {
    self.glkViewController = [[PanoramaVC alloc] init];
...
- (void)initScrubberTimer {
    ...
    [self retrievePixelBufferToDraw];
...
- (void)observeValueForKeyPath:(NSString*)path
    ...
    if (context == AVPlayerDemoPlaybackViewControllerStatusObservationContext) {
        [self updatePlayButton];
        AVPlayerStatus status = AVPlayerStatusUnknown;
