...
//        glClearColor(0.3f, 0.6f, 1.0f, 1.0f);
    ...
    }else if(_srcType == ZLCPanoSRCTypeStream) //视频流渲染
    {
        ...
        if(pixelBuffer!= NULL){
            renderer.processUI([self cvMatFromUIImage:[self uiImageFromPixelBuffer:pixelBuffer]]);
...
int esGenSphere(int numSlices, float radius, float **vertices,
    ...
    if (vertices != NULL) {
        *vertices = static_cast<float*>(malloc(sizeof(float) * 3 * numVertices));
    }
    
    if (texCoords != NULL) {
        *texCoords = static_cast<float*>(malloc(sizeof(float) * 2 * numVertices));
    }
    
    if (indices != NULL) {
        *indices = static_cast<uint16_t*>(malloc(sizeof(uint16_t) * numIndices));
    }
...
//渐变动画参数
    ...
    NSLog(@"========%f,%f======",n_fingerX*100, n_fingerY*100);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    NSLog(@"+++++++%f,%f",self.fingerRotationX,self.fingerRotationY);
    renderer.handleTouchDrag(distX*5,distY*5);
...
//    path = [NSString stringWithFormat:@"%@/img.jpg",path];
